...
public class WebCrawler7 {
	...
	public static void main(String[] args) {
		...
		String outputPath="/home/pasha/Desktop/sem6/db_lab_CS387/lab5/outlab/outputs/output_P1";
		int max_hops = 3;
		...
		while(max_hops > 0) {
			...
			to_crawl = to_crawl.flatMap((FlatMapFunction<Row, Row>) row -> {
				...
				try {
					...
					Connection.Response res = Jsoup.connect(abs_url).execute();
					if(res.contentType().equals("text/html")) {
			...
			cumulative = cumulative
					.union(to_crawl)
					.groupBy("abs_url")
					.agg(functions.max("crawled").as("crawled"))
					.cache();
			
			max_hops--;
			System.out.println(max_hops);
		}
		cumulative.select("abs_url").toJavaRDD().saveAsTextFile(outputPath);
	}
